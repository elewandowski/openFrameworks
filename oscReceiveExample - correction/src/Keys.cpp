...
void Keys::initBuffer() {
    for(int i=0; i<BUFFER_SIZE; i++) {
        audioR[i], audioL[i] = 0;
    }
}

void Keys::updateBuffer() {
    if(UPDATE_BUFFER) {
        for (int bufferPos=0; bufferPos<BUFFER_SIZE; bufferPos++) {
            audioArray[writeHeadAudioArray][bufferPos] = getSampleFromBuffer(bufferPos);
        }
        writeHeadAudioArray = (writeHeadAudioArray + 1) % numOfBuffers;
    }
}
